...
## 결정됨 (2026-06-17)
- **사이드바**: 데스크탑은 좌측 날짜 사이드바 **유지**, **모바일은 ☰ 드로어(오버레이)**.
- **날짜 이동**: 상단에 **‹ › 양쪽 화살표**로 인접 거래일 이동(사이드바 목록과 별개, home·상세 공통).
  dates[]는 최신순(DESC) → ‹ = 과거(index+1), › = 미래(index-1), 양 끝에서 비활성.
- **네비(4)**: SPA 라우터 없이 상태(`view`) 기반 전환 + 상세 상단 "← 홈" 버튼.
- **색(6)**: accent 무채색(엷은 회색 `#d4d4d4`).
- **요약 칩(5)**: home에서도 ‹ › 로 날짜 바꾸면 요약 갱신(선택 날짜 다이제스트 기준).
