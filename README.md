...
명령어도 사진도 아닌 일반 텍스트는 **개인 주식 비서** 대화입니다: 규칙서·봇 사용법에
더해 **현재 운용 상태**(마지막 확인 잔고·비중·가속 플래그·최근 판정 기록)를 알고
답하며, 웹 검색으로 뉴스·시장 정보도 종합해줍니다. 잡담도 됩니다. 단 집행 가드레일은
유지 — 뉴스를 근거로 한 매매 제안, 규칙 밖 매매 상담, 새로운 돈 계산은 하지 않습니다
(`backend/reply.py`). 대화는 `claude --resume` 으로 **세션이 유지**되고(`/newchat` 으로
초기화), 세션은 도커 볼륨(`claude-home`)에 보존되어 재배포에도 살아남습니다.
