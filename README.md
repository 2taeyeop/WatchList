...
명령어도 사진도 아닌 일반 텍스트는 규칙서·봇 사용법을 지식원으로 클로드가 답합니다
(전망·추천 거절, 규칙 밖 매매는 개정 절차 안내 — `backend/reply.py` 가드레일).
자유 질문은 `claude --resume` 으로 **대화 세션이 유지**되어 이어서 질문할 수 있고,
세션은 도커 볼륨(`claude-home`)에 보존되어 재배포에도 살아남습니다.
