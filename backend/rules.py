...
TARGET_QLD = 0.70             # 목표 비율 QLD 70 : JEPI 30
...
ALLOWED_STEADY = {"QLD", "JEPI"}
ALLOWED_ENTRY = {"QLD", "JEPI", "SGOV"}
...
class Snapshot:
    ...
    @property
    def qld_value(self) -> float:
        return self.qld_shares * self.qld_price
    ...
    def total_value(self) -> float:
        return self.qld_value + self.jepi_value

    @property
    def qld_weight(self) -> float:
        total = self.total_value
        return self.qld_value / total if total else 0.0
...
def _weight(qld_shares: int, jepi_shares: int, qld_price: float, jepi_price: float) -> float:
    total = qld_shares * qld_price + jepi_shares * jepi_price
    return qld_shares * qld_price / total if total else 0.0


# ---------- A. 월간 적립 ----------
def judge_monthly(snap: Snapshot, avail_usd: float) -> Judgment:
    """비중<70% → 가용달러 전액 QLD 내림 매수, 아니면 전액 JEPI. 잔돈은 이월."""
    w = snap.qld_weight
    buy_qld = w < TARGET_QLD
    ticker, price = ("QLD", snap.qld_price) if buy_qld else ("JEPI", snap.jepi_price)
    shares = _floor_div(avail_usd, price)
    spent = shares * price
    after = _weight(snap.qld_shares + (shares if buy_qld else 0),
                    snap.jepi_shares + (0 if buy_qld else shares),
                    snap.qld_price, snap.jepi_price)
    orders = (Order(ticker, "BUY", shares, spent),) if shares > 0 else ()
    return Judgment(
        action=f"월간적립 {ticker} {shares}주",
        reason=f"QLD 비중 {w:.1%} → {'70% 미만이라 QLD' if buy_qld else '70% 이상이라 JEPI'} 매수. "
...
def judge_accel(snap: Snapshot, drawdown: float, tier1_fired: bool, tier2_fired: bool,
    ...
    for tier, threshold, already in (("1단", TIER1_DD, tier1_fired), ("2단", TIER2_DD, tier2_fired)):
        ...
        buy = _floor_div(proceeds, snap.qld_price)
        orders.append(Order("JEPI", "SELL", sell, sell * snap.jepi_price))
        orders.append(Order("QLD", "BUY", buy, buy * snap.qld_price))
        carry += proceeds - buy * snap.qld_price
        jepi -= sell
        qld += buy
    ...
    return Judgment(
        ...
        reason=f"하락률 {drawdown:.1%} — {'·'.join(fired)} 기준 충족. "
               f"JEPI 절반 매도 대금 전액 QLD 매수, 올해 12월 리밸런싱 스킵",
        orders=tuple(orders),
        carry_delta_usd=carry,
        weight_before=snap.qld_weight,
        weight_after=_weight(qld, jepi, snap.qld_price, snap.jepi_price),
...
def judge_december(snap: Snapshot, year: int, skip_december_year: int | None) -> Judgment:
    if skip_december_year == year:
        return Judgment(
            action="12월 리밸런싱 스킵",
            reason=f"{year}년 가속 발동 연도 — 바닥에서 QLD를 되파는 자기모순 방지 규칙",
            weight_before=snap.qld_weight, weight_after=snap.qld_weight,
        )

    w = snap.qld_weight
    target = _floor_div(snap.total_value * TARGET_QLD, snap.qld_price)
    diff = target - snap.qld_shares
    if abs(diff) <= 1:
        return Judgment(
            action="12월 리밸런싱 패스",
            reason=f"목표 QLD {target}주 vs 현재 {snap.qld_shares}주 — 차이 {abs(diff)}주(≤1주)",
    ...
    if diff < 0:
        sell = -diff
        proceeds = sell * snap.qld_price
        buy = _floor_div(proceeds, snap.jepi_price)
        orders.append(Order("QLD", "SELL", sell, proceeds))
        orders.append(Order("JEPI", "BUY", buy, buy * snap.jepi_price))
        carry = proceeds - buy * snap.jepi_price
        after = _weight(target, snap.jepi_shares + buy, snap.qld_price, snap.jepi_price)
    else:
        need = diff * snap.qld_price
        sell = min(snap.jepi_shares, _ceil_div(need, snap.jepi_price))
        proceeds = sell * snap.jepi_price
        # 목표는 70:30 복원이지 QLD 초과 매수가 아니므로 diff 주로 상한.
        buy = min(diff, _floor_div(proceeds, snap.qld_price))
        orders.append(Order("JEPI", "SELL", sell, proceeds))
        orders.append(Order("QLD", "BUY", buy, buy * snap.qld_price))
        carry = proceeds - buy * snap.qld_price
        after = _weight(snap.qld_shares + buy, snap.jepi_shares - sell,
                        snap.qld_price, snap.jepi_price)
    return Judgment(
        action="12월 리밸런싱",
        reason=f"목표 QLD {target}주 vs 현재 {snap.qld_shares}주 — 차이 {diff:+}주",
...
def judge_withdraw(need_krw: float, fx: float, snap: Snapshot,
                   qld_gain_krw_per_share: float | None = None,
                   jepi_gain_krw_per_share: float | None = None) -> Judgment:
    """필요 원화 → 달러 환산 → QLD 70 : JEPI 30 금액 비율 매도. 정상 절차."""
    ...
    plans = (("QLD", need_usd * TARGET_QLD, snap.qld_price, snap.qld_shares, qld_gain_krw_per_share),
             ("JEPI", need_usd * (1 - TARGET_QLD), snap.jepi_price, snap.jepi_shares, jepi_gain_krw_per_share))
    orders: list[Order] = []
    ...
    return Judgment(
        ...
        orders=tuple(orders),
        weight_before=snap.qld_weight,
        weight_after=_weight(snap.qld_shares - sold["QLD"], snap.jepi_shares - sold["JEPI"],
                             snap.qld_price, snap.jepi_price),
    )


# ---------- 진입기(ENTRY) 주간 루틴 ----------
def judge_entry(sgov_shares: int, entry_week: int, sgov_price: float, qld_price: float) -> Judgment:
    """이번 주차의 SGOV 매도 → QLD 매수. 5주차는 SGOV 잔량 전부."""
    ...
    buy = _floor_div(proceeds, qld_price)
    ...
    return Judgment(
        ...
        orders=(Order("SGOV", "SELL", sell, proceeds),
                Order("QLD", "BUY", buy, buy * qld_price)),
        carry_delta_usd=proceeds - buy * qld_price,
        state_updates=updates,
    )
