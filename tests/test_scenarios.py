...
def payload(tq=50, je=100, tp=70.0, jp=55.0, dd=-0.03, cash_usd=1000.0,
    ...
    return {
        "extracted": {
            "positions": [
                {"ticker": "QLD", "shares": tq, "pnl_krw": pnl_tq},
...
def test_scenario_normal_monthly(bot):
    ...
    assert "월간적립 QLD 14주" in text          # floor(1000/70)
...
def test_scenario_first_tier1_at_minus_26(bot):
    ...
    assert "QLD | 매수 | 39주" in text          # floor(2750/70)
    state = db.get_state()
...
def test_scenario_mismatch_stops_at_gate():
    ...
    data = {"positions": [{"ticker": "QLD", "shares": None, "name_raw": "QLD"}],
...
def test_null_required_fields_block_at_gate():
    ok_positions = [{"ticker": "QLD", "shares": 10, "pnl_krw": None}]
    ...
    # 값이 다 있으면 통과
    full = {"positions": [{"ticker": "QLD", "shares": 10, "pnl_krw": 1000}],
